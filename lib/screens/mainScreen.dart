...
class _MainScreenState extends State<MainScreen> {
  ...
  checkLoginStatus() async {
    ...
    Timer.periodic(Duration(seconds: 2), (timer) {
      setState(() {
        if (sharedPreferences.getBool('isLogout') != null) {
          isLogout = sharedPreferences.getBool('isLogout')!;
        }
        if (expirytimes.isBefore(DateTime.now()) || isLogout) {
          sharedPreferences.remove('token');
          sharedPreferences.remove('isLogout');
